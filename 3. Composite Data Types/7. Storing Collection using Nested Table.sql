/* Storing Collection - Part 2 - Using Nested Tables */
/*
Why we use nested table over varrays? We can extend the size.
WHat if we again change size (increase the size) using
create or replace type hr.v_phone_numbers as varray(10) of t_phone_number; ??
-> We cannot, gives error cannot drop or replace a type with type or table dependents
-> b/c it has already been used by the tables. So, if there's any possibility of extension in size, we need to use nested tables
Cons: We need to write a PL/SQL for updating the records in nested table stored as a column in a table
*/
