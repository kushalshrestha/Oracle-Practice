...
/*
    ...
    - keys does not need to be sequential, can be negative
    - can have scalar & record types
    - do not need to initialize associative arrays, unlike varrays and nested tables
    - associative arrays are indexed
    - we can create associative like with table but instead of getting from database, memory is faster
    -- !!! Can be PLS_INTEGER | BINARY_INTEGER | VARCHAR2(size)
    -- !!! cannot create in schema level, unlike varrays and nested tables. we can create only in the memory
*/
