/* EXECUTE IMMEDIATE STATEMENT with USING clause */

/* The below code at the bottom will show how dynamic sql can lead to error. So the solution could be using 'INTO' clause
i.e EXECUTE IMMEDIATE Statement with USING + INTO clause
*/
...
END;
-- will give error invalid host/bind variable name. i.e if your statement is incorrect your code is at risk
/
CREATE OR REPLACE FUNCTION update_names (ID IN VARCHAR2, last_name IN VARCHAR2, first_name OUT VARCHAR2) RETURN PLS_INTEGER IS
    v_dynamic_sql VARCHAR2(200);
BEGIN
